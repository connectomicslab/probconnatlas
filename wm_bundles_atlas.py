...
def _build_args_parser():
    ...
    requiredNamed.add_argument(
        ...
        action="store",
        required=False,
        choices=["scale1", "scale2", "scale3", "scale4"],
        metavar="SCALE",
        type=str,
        nargs=1,
    ...
    if method == "connectivity" and not os.path.isfile(map):
        ...
        print(" ")
        print(
            "Please, supply a valid txt or binary mask to select the bundles. Use the flags --bfile or -f."
        )
        print(" ")
        print(" ")
...
def bundles_from_mask(
    ...
    if boolcoll:
        collMask = np.zeros(
            (dim[0], dim[1], dim[2]), dtype="float"
        )  # Collapsing all the bundles
    ...
    for i in np.arange(0, tbund - 1):
        ...
        if savebund:
            ...
            if boolbund:
                bundFilename = os.path.join(
                    outDir, fname + "-fromMask-" + tempS + "-" + scaleId + ".nii.gz"
                )
        ...
        printProgressBar(
            i + 1,
            tbund,
            "Processing bundle: " + "(" + str(i + 1) + "/" + str(tbund) + ")",
        )
    ...
    if booltab:
        ...
        with open(tabFilename, mode="w") as bundTab:
            bund = csv.writer(
                bundTab, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            for i in np.arange(len(tabList)):
                bund.writerow(tabList[i])
    # Saving the collapsed image
    if boolcoll:
        collFilename = os.path.join(
            outDir, fname + "-fromMask-bundcollap-" + scaleId + ".nii.gz"
        )
        # collMask        = collMask != 0
...
def bundles_from_textfile(
        ...
        if tempVar != None:
            ...
            if np.any(array_data):
                if boolcoll:
                    collMask = collMask + np.asarray(array_data > 0)
                    collMaskc[:, :, :, 0] = (
                        collMaskc[:, :, :, 0] + array_data * bdcolor[0]
                    )
                    collMaskc[:, :, :, 1] = (
                        collMaskc[:, :, :, 1] + array_data * bdcolor[1]
                    )
                    collMaskc[:, :, :, 2] = (
                        collMaskc[:, :, :, 2] + array_data * bdcolor[2]
                    )
                ...
                if booltab:
                    ...
                    tabList.append(
                        [
                            str(bundIds[i, 0]) + "_" + str(bundIds[i, 1]),
                            sourROI,
                            targROI,
                            cVal,
                            mVal,
    ...
    if boolcoll:
        collFilename = os.path.join(
            outDir, fname + "-fromCSV-bundcollap-" + scaleId + ".nii.gz"
        )
        collMaskc[:, :, :, 0] = np.divide(collMaskc[:, :, :, 0], collMask, where=True)
        collMaskc[:, :, :, 1] = np.divide(collMaskc[:, :, :, 1], collMask, where=True)
        collMaskc[:, :, :, 2] = np.divide(collMaskc[:, :, :, 2], collMask, where=True)
...
def bundles_to_connectivity(
    ...
    if (
        os.path.isfile(outBasename + "-connmat-mean-" + scaleId + ".h5")
        and boolforce == True
    ):
        print(
            " The file "
        ...
        print("  ")
    if (
        os.path.isfile(outBasename + "-connmat-std-" + scaleId + ".h5")
        and boolforce == True
    ):
        print(
            " The file "
        ...
        print("  ")
    if (
        os.path.isfile(outBasename + "-connmat-median-" + scaleId + ".h5")
        and boolforce == True
    ):
        print(
            " The file "
    ...
    medMat = np.zeros(
        [nstr, nstr]
    )  # Number of Points. Important to compute the mean along fibers
    ...
    for i in np.arange(0, tbund - 1):
        ...
        if np.any(array_data):
            ...
            meanMat[int(Y[i]) - 1, int(X[i]) - 1] = meanMat[
                int(X[i]) - 1, int(Y[i]) - 1
            ]
...
def main():
    ...
    if fmeth == "connectivity":
        bundles_to_connectivity(
            hfName, fout, scaleid, fmap, voxth, subth, boolbund, boolforce
        )
        ...
        if imask == True:
            ...
            bundles_from_textfile(
                hfName, file, fout, scaleid, fmap, voxth, subth, boolbund, boolcoll
            )
    else:
